...
def do_work(db_exp, args_exp, exp, args_cntrl, cntrl, bdate, edate):
    ...
    # OPS = 8 days
    # EXP = 5 days
    fcst_length = range(24, 5*24+1, 12)
    #fcst_length = [4*24-12, 4*24, 4*24+12, 5*24]
    if 'ops' in db_exp and 'ec' not in args_exp and 'ec' not in args_cntrl and 'gfs' not in [args_exp, args_cntrl] and 'fpp' not in args_exp and 'fpp' not in args_cntrl and 'fp' not in args_exp and 'fp' not in args_cntrl:
